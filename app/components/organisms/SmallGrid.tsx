...
import { SmallGridItem } from "../molecules/GridItems";
...
const SmallGrid: React.FC<GridProps> = ({ headerText, data }) => {
  ...
  return (
    <div className="mb-10 px-4 md:px-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:mx-4">
        {itemsToShow.map((gridObj) => <SmallGridItem key={gridObj.id} gridObj={gridObj} href={`/recipe/${gridObj.id}`} /> )}
      </div>
    </div>
  );
};
